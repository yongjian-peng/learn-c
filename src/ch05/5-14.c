 ...
 * @brief  修改排序程序，使它能处理-r标记，该标记表明，以逆序递减方式排序，要保证-r和-n能够组合在一起使用
 * 添加-f标记，使得排序过程不考虑字符大小写之间的区别，例如，比较 a和A时认为它们是相等的
...
int charcmp(const char *s, const char *t)
{
    for(; *s && *t && tolower(*s) == tolower(*t); s++, t++)
        ;
    if (tolower(*s) < towlower(*t))
        return -1;
    else if(tolower(*s) > towlower(*t))
        return 1;
    else    
        return 0;
}
